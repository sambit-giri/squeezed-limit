...
double Cosmology::cosmicTime(double zCurrent) 
{
  ...
  if ((omega0 + lambda0) == 1.0) {          // Peebles 1994 13.20
...
double MfromTvir(Cosmology *c, double tv, double z, double mu)
{
  ...
  ans = 6950.0*pow(tv/mu,1.5)/pow(1.0+z,1.5);
  ans *= sqrt(c->omegaZ(z)/c->getOm0hh()/c->DeltaC(z));
  return ans;
}
...
/* Calculates Sheth-Tormen (1999) mass function in logarithmic units
 * (per comoving Mpc^-3).
 */
...
{
  const double a(0.7),beta(0.485),alpha(0.615);
  double dCritZ,sigM,dlsdlM,tdn,dsdM,nuc;
  double barrier_sig;
  ...
  dlsdlM = tM*dsdM/sigM;
  nuc = dCritZ/sigM;
  barrier_sig = sqrt(a)*nuc*(1.0+beta*pow(a*nuc*nuc,-alpha));

  tdn = sqrt(2.0*a/PI)*fabs(dlsdlM)/tM*nuc;
  tdn *= (1.0+0.0763*pow(a*nuc*nuc,-alpha));
  tdn *= exp(-barrier_sig*barrier_sig/2.0);
  tdn *= CRITDENMSOLMPC*c->getOm0hh();
  return tdn;
}

/* Conditional Sheth-Tormen halo mass function, in logarithmic units 
 * (per comoving Mpc^-3) */
		 ...
		 Cosmology *c)
{
  ...
  sFac = sigM*sigM - sigB*sigB;
  sM2 = sigM*sigM;
  kernel = a*dCritZ*dCritZ;

  TsS = pow(sM2/kernel,alpha);
  TsS += alpha*pow(-sFac/kernel,1.0)*pow(sM2/kernel,alpha-1.0);
  TsS += alpha*(alpha-1.0)*pow(-sFac/kernel,2.0)*pow(sM2/kernel,alpha-2.0)/2.0;
  TsS += (alpha*(alpha-1.0)*(alpha-2.0)*pow(-sFac/kernel,3.0)*
	  pow(sM2/kernel,alpha-3.0))/6.0;
  TsS += (alpha*(alpha-1.0)*(alpha-2.0)*(alpha-3.0)*pow(-sFac/kernel,4.0)*
	  pow(sM2/kernel,alpha-4.0))/24.0;
  TsS += (alpha*(alpha-1.0)*(alpha-2.0)*(alpha-3.0)*(alpha-4.0)*
	  pow(-sFac/kernel,5.0)*pow(sM2/kernel,alpha-5.0))/120.0;
  TsS *= beta;
  TsS += 1.0 - deltaBubble/sqrt(kernel);

  barrier = sqrt(kernel)*(1.0+beta*pow(kernel/sM2,-alpha));

  tdn = sqrt(2.0*a/PI)*fabs(dlsdlM)/tM*dCritZ*sM2/pow(sFac,1.5);
  tdn *= TsS;
  tdn *= exp(-pow(barrier-deltaBubble,2.0)/sFac/2.0);
  tdn *= CRITDENMSOLMPC*c->getOm0hh();
  return tdn;
}

/* Halo bias (linear) for Press-Schechter mass function */
...
/* Halo bias (linear) for Sheth-Tormen mass function */
double biasm_st(double mass, double z, Cosmology *c)
{
  const double a(0.707),p(0.3);
  double deltasc,deltasc0,sig;
  double bias;
  double nuc;
  ...
  deltasc = c->delCrit0(z)/c->growthFac(z);
  deltasc0 = c->delCrit0(0.0);
  sig = sigm(mass,nuc,c,0);
  nuc = a*pow(deltasc/sig,2.0);
  bias = 1.0 + (nuc - 1.0)/deltasc0;
  bias += 2.0*p/deltasc0/(1.0+pow(nuc,p));
  return bias;
}
