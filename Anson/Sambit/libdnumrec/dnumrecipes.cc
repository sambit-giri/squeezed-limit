...
double qromb(double (*func)(double), double min, double max, 
	     double acc)
{
  ...
  *nbad = 0;
  ...
  for (nstp=1;nstp<=MAXSTP;nstp++) {
    ...
    for (i=1;i<=nvar;i++) {
      ...
      //      std::cout << "yscal[i] = " << yscal[i] << std::endl;
    }
	...
	ystart[i]=y[i];
      ...
      std::cout << "Exception processed in odeint: " << ex.what() << std::endl;
      throw;
    }
    if (hdid==h) { 
      ++(*nok);
...
//approximate function in interval [a,b]
//use n ~50.  
...
//xlow and xhigh give the intervals over which to fit the Ch. polynomials
//order goes up to second derivative
double ch_derivative(double (*func)(double), double x, double xlow, 
		     double xhigh, int npoly, int order, double &a)
{
  double c[npoly];
  double cder[npoly];

  chebft(xlow,xhigh,c,npoly,func);
  chder(xlow,xhigh,c,cder,npoly);

  if(order == 1)
    return  chebev(xlow,xhigh,cder,npoly,x);
  else if(order == 2)
    {
      double cder2[npoly];
      chder(xlow,xhigh,cder,cder2,npoly);
      a = chebev(xlow,xhigh,cder2,npoly,x);
      return chebev(xlow,xhigh,cder,npoly,x);
    }
  else
    {
      std::cout <<  "order must be 1 or 2 in ch_derivative." << std::endl;
      exit(-1);
    }
  return 0;
}
